            ...
            function authThen () {
                ...
                if ($rootScope.toState.data.authorities && $rootScope.toState.data.authorities.length > 0 && !Principal.hasAnyAuthority($rootScope.toState.data.authorities)) {
                    if (isAuthenticated) {
                        ...
                        // altered sign-in so it has it's own state at the app root and is no longer a modal
                        // so we don't need to redirec to to accessdenied, just redirect to login.
                        $state.go('login');
                    }
                }
            }
